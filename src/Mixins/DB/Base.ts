 ...
 */
export function Base<TService extends Service<ISettings> & { prisma: unknown, model: unknown }>(client: new () => TService["prisma"], modelName: keyof TService["prisma"]): ServiceSchema<TService> {
  return {
    ...
    async started() {
      this.prisma = (this as unknown as { createPrismaClient(client: unknown): TService["prisma"] }).createPrismaClient(client)
      this.model = (this.prisma as Record<keyof TService["prisma"], unknown>)[modelName]
    },
